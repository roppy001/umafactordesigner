...
| カテゴリ | スキルID範囲 | 説明 |
|---------|------------|------|
| 本体固有 | 10000番台 | 各キャラの固有スキル（継承時の効果値を使用） |
| 進化固有 | 100000番台 | 進化後の強化版固有スキル |

### 親進化条件（手動補足）

以下の2キャラクターは、**親として使用した場合に特定の固有スキルが進化する**。
この条件はU-toolsに記載がないため、JSONに手動で補足すること。

| 親キャラクター | 備考 |
|--------------|------|
| ステイゴールド | 特定の継承先のスキルが進化する |
| [Eternal Fairytale] ラインクラフト | 特定の継承先のスキルが進化する |
...
| `category` | string | `"固有"` または `"進化固有"` |
| `activation_conditions` | object | 発動条件 |
| `effects` | object | 本人使用時の効果 |
| `inherited_effects` | object | 継承時（200pt取得時）の効果 |
...
```
output/{レース場}_{路面}_{距離}m.md
```

例: `output/東京_芝_2400m.md`
